...
contract_names_per_file = [
    ...
    {"file": "buggy_10.sol", "names": ["Ownable", "XLToken"]},
    {
        "file": "buggy_11.sol",
        "names": [
            "ERC20Interface",
    ...
    {
        "file": "buggy_16.sol",
        "names": ["ERC20Interface", "Owned", "ExclusivePlatform"],
    },
    ...
    {
        "file": "buggy_27.sol",
        "names": ["Ownable", "ERC20Detailed", "DanPanCoin"],
    },
    ...
    {
        "file": "buggy_40.sol",
        "names": ["ERC20", "ERC20Detailed", "SimpleSwapCoin"],
    },
    ...
    {
        "file": "buggy_47.sol",
        "names": ["ERC20Interface", "AcunarToken", "AcunarIEO"],
    },
    {
        "file": "buggy_48.sol",
        "names": [
...
def evaluate_tools():
    ...
    for tool in tools:
        for cs in x:
            tool_bugs = [
                bugs["bugs"] for bugs in bug_types if bugs["tool"] == tool
            ]
            for bug_type in tool_bugs[0]:
                time = solidifi.interior_main(
                    "-i", "contracts/" + str(cs) + ".sol", bug_type
                )
            ...
            for buggy_sc in glob.glob(injected_scs + "/*.sol"):
                ...
                if tool == "Oyente":
                    ...
                    tool_cmd = 'docker run -i -t -v {0} luongnguyen/oyente bash -c " cd oyente ; python oyente.py -ce -j -s ../contracts/{1} " >{2} '.format(
                        os.path.join(os.getcwd(), injected_scs)
                        + ":/oyente/contracts",
                        tail,
                        result_file,
                    )
                    ...
                    tool_cmd = "smartcheck -p {0} > {1}".format(
                        buggy_sc, result_file
                    )
                    os.system(tool_cmd)
                    ...
                    # Slither command
                    tool_cmd = "slither  {0} --json {1}".format(
                        buggy_sc, result_file
                    )
                    os.system(tool_cmd)
