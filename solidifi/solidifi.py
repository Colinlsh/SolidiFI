...
from .inject_file import adjust_injected_loc, get_pattern_all_offsets, get_pattern_offset, get_snippet_at_offset, preprocess_json_file, update
...
class Solidifi():
    ...
    def inject(self, file_path: str, file_name_with_ext: str, bug_type: str, output_dir: str) -> None:
        ...
        tmp_buggy_file_path = os.path.join(buggy_dir,"tmp_buggy_"+file_name_with_ext)
        ...
        stdout, stderr, exit_code = run_subprocess(f"solc --ast-json {self.cur_contr_file} -o {ast_json_files_dir} --overwrite")
        if exit_code != 0:
            ...
            destination_file = os.path.join(destination_folder, os.path.basename(self.cur_contr_file))
        ...
        with open(ast_json_file) as fh:
            ...
            if file_contents.strip():
                ...
                print(f"File '{ast_json_file}' is empty or contains only whitespace.")
                self.cur_contr_ast_data = None
    
        self.inject_bug(bug_type)
        csv_file = os.path.join(buggy_dir,"BugLog_"+file_name_with_ext[0:len(file_name_with_ext)-4]+".csv")
    ...
    def inject_bug(self, bug_type):
        ...
        cur_bug_dir = os.path.join(self.bugs_dir , cur_bug_type_details[0]['bug_type_dir'])
        ...
        for bug_forms in ('s', 'f'):
            ...
            for loc in reversed(BIP):
                if  not bug_seq < len(bugfiles):
                    ...
                    break
                bug_f: BufferedReader = open(os.path.join(cur_bug_dir,bugfiles[bug_seq]), "rb")
                bug_snip = bug_f.read()
                ...
                stm = get_snippet_at_offset(self.src_contr_file, soffset, stm_size)
                ...
                if (loc['name'] in ['VariableDeclaration','ExpressionStatement','Identifier','EmitStatement','PlaceholderStatement','Return','EventDefinition'] 
                    and (new_loc[0] not in self.BugLog) and (soffset not in injected_loc_src_mapping)):
                    update(self.cur_contr_file, new_loc[0], bug_snip.strip()+b'\n')
                    self.BugLog = adjust_injected_loc(self.BugLog,new_loc[2], bug_snip_len)
                    self.BugLog.append({'loc':new_loc[2],'length':bug_snip_len,'bug type':bug_type,'approach':'code snippet injection'})
                    injected_loc_src_mapping.append(soffset)
                    bug_seq +=1 
                elif (loc['name'] in ['Block', 'FunctionDefinition', 'ModifierDefinition'] and (new_loc[1] not in self.BugLog) and (eoffset not in injected_loc_src_mapping)):
                    
                    update(self.cur_contr_file, new_loc[1]+2, b'\n'+bug_snip.strip())
                    self.BugLog = adjust_injected_loc(self.BugLog, new_loc[2]+2, bug_snip_len)
                    self.BugLog.append({'loc':new_loc[2]+1,'length':bug_snip_len,'bug type':bug_type,'approach':'code snippet injection'})
    ...
    def get_potential_locs(self, ast, bug_snip_type):
        ...
        interface_locs = get_pattern_all_offsets(self.src_contr_file,'interface')
        interface_locs.extend(get_pattern_all_offsets(self.src_contr_file,'library'))
        ...
        for stm_type in stm_types: 
            ...
            if stm_type in ('VariableDeclaration','Identifier'):
                for vr in sub_stm:
                    if not self.within_interface(interface_list, interface_locs, int(self.get_src(vr['src'])['eoffset'])) and not self.is_paramter(param_list, int(self.get_src(vr['src'])['eoffset'])) and not self.within_struct_block(struct_list, int(self.get_src(vr['src'])['eoffset'])):
                        stmt_locs.append(vr)
                ...
                for stm in sub_stm:
                    if not self.within_interface(interface_list, interface_locs, int(self.get_src(stm['src'])['eoffset'])) and not self.within_main_block(block_list, int(self.get_src(stm['src'])['eoffset'])):
                        stmt_locs.append(stm)
            else:
                for stm in sub_stm:
                    if not self.within_interface(interface_list, interface_locs, int(self.get_src(stm['src'])['eoffset'])):                    
                        stmt_locs.append(stm)
        ...
        with open (filename, "r+b") as myfile:
            ...
            for bug in bug_patterns:
                ...
                if (len(bug_locs) >0):
                    ...
                    myfile.truncate()   
                for loc in bug_locs:
                    self.BugLog.append({'loc':loc['line'], 'length':1,'bug type':bug_type, 'approach':'code tranfsorm'})
            ...
            if (len(bug_locs) >0):
                for loc in bug_locs:
                    with open(filename,'r+b') as sfile:
                        ...
                        sfile.truncate()
                    self.BugLog.append({'loc':loc['line'], 'length':1,'bug type':bug_type, 'approach':'weakening security'})
