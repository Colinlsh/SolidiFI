...
class BugInjector():
      ...
      def inject(self, file_path: str, bug_type: BugType, output_path: str = "test/output") -> None:
            try:
      ...
      def inject_multiple_concurrently(self, directory: str, bug_type: BugType, output_path: str = "test/output", num_of_process: int = -1) -> None:
      ...
      def _inject_it(self, chunk, bug_type, output_path):
            return [self.inject(file_path=Path(file_path), bug_type=bug_type, output_path=Path(output_path)) for file_path in chunk]
                  
      def _get_bug_info(self, bug_type: BugType, config_path: str="bug_types.conf") -> Optional[BugInfo]:
            config = ConfigParser()
